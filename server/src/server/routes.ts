...
router.post("/submit", async (req , res) => {
  ...
  const { firstName, lastName, studyField, email ,phone } = req.body;
  console.log("Hello register")
  /**
   * Input validation (VERY IMPORTANT)
   * Never assume the client sends correct data
   */
  if (!firstName || !lastName || !studyField || !email || !phone) {
    return res.status(400).json({
      message: "כל השדות רישום הם חובה",
    });
  }

  console.log("נתונים שהתקבלו:", { firstName, lastName, studyField, email, phone });
